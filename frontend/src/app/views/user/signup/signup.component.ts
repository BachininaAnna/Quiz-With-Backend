...
export class SignupComponent implements OnInit {
  signupForm = new FormGroup({
    ...
    lastName: new FormControl('', [Validators.pattern(/^[A-Z][a-z]+s*/), Validators.required]),
    email: new FormControl('', [Validators.email, Validators.required]),
    password: new FormControl('',
      [Validators.pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/), Validators.required]),
        ...
        .subscribe({
          next: (data: SignupResponseType) => {
            if (data.error || !data.user) {
              ...
              throw new Error(data.message ? data.message : 'Error with data with signup');
            }
                ...
                .subscribe({
                  next: (data: LoginResponseType) => {
                    if (data.error || !data.accessToken || !data.refreshToken
                      ...
                      this._snackBar.open('Ошибка при авторизации попробуйте ещё раз');
